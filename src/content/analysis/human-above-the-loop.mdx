...
excerpt: "You don't put the CEO in the assembly line. You put the CEO above it, setting policy and reviewing outcomes."
date: 2026-08-30
tags: ["autonomous-agents", "ai-security"]
...
- [Zero Trust Applied to AI Systems](/research/zt-ai-systems) — the ZTA architecture that human-above-the-loop policy authoring sits inside. Policy decision points don't approve individual requests; they author the rules.
- [Dual-Intent Runtime Authorization (DIRA)](/research/dira-framework) — the runtime mechanism that enforces the policies humans author. The "above the loop" model only works if the enforcement layer is actually checking intent.
- [Non-Human Identity for Agentic Systems](/research/nhi-agentic) — agents need governed identities before humans can author meaningful policies about them.
- [CB4A: content-based 4-tier authorization for AI tool calls](/build-logs/cb4a-implementation) — what the policy enforcement point looks like in production. T1-T2 fast path, T3-T4 human escalation.
- [NIST AI RMF in practice](/analysis/nist-ai-rmf-practice) — the governance framework that human-above-the-loop sits inside. The Govern function is where policy authoring lives.
