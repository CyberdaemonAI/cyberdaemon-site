...
excerpt: "Vague input plus a capable model does not give you a vague result. It gives you a confident, well-formatted, fully-tested result that solves a problem adjacent to yours."
date: 2026-08-31
...
- [ISA-driven development: structured contracts for autonomous AI agents](/build-logs/isa-patterns) — ISAs are the operationalized form of the specification skill. This article describes why specification matters; ISA patterns describe how to write specifications that agents can actually execute.
- [My AI system builds itself, that's the point](/build-logs/ai-system-builds-itself) — an agent writing the specification for how agents write articles is the concrete version of "specification is the bottleneck." The spec quality determined whether the fleet ran.
- [Agent-written CI: 18 of 19 tests](/build-logs/ci-gate-agent-written) — what happens when the specification is precise enough for an agent to write the verification layer. 18 of 19 passing is a specification outcome, not a coding outcome.
- [Writing to the Next Session](/build-logs/writing-to-the-next-session) — session handoff as a specification problem. The recovery document is a specification the next agent reads; its quality determines whether recovery succeeds.
