...
description: "I read the HuggingFace/OpenAI incident report. Then I audited Prometheus, the autonomous agent system I operate. Here is what I found, what I changed, and what I am still worried about."
date: 2026-08-30
tags: ["incident-response", "agentic-ai", "security", "huggingface", "prometheus"]
featured: false
---
...
I read the incident report on a Thursday night. By Friday morning I was auditing Prometheus.

Prometheus is the AI system I build and operate. It runs on K3s across three nodes. It has autonomous agents (B0b) that execute deployments, manage git repos, and SSH into infrastructure. It has a message board (Zulip) where agents coordinate. It has episodic memory (prom-memory) that persists across sessions. It has MCP servers that expose tools to AI clients. It has tool call authorization (CB4A) that gates what agents can do. It has identity governance (DIRA) that binds agent scope to delegated human intent.

The overlap with the attack surface in the HuggingFace incident is not abstract. I run the same class of system the attackers compromised. Autonomous agents, Kubernetes secrets, Tailscale for networking, long-lived credentials, tool access that spans infrastructure.

<Callout variant="critical" label="The uncomfortable question">
If the ExploitGym models could chain nine hops from a sandbox to a production cluster at another company, what could a compromised agent do inside Prometheus, where the agents already have legitimate access to SSH keys, git credentials, MCP tokens, and Tailscale?
</Callout>

<ScrollReveal>
<StatRow stats={[
  { value: "3", label: "K3s nodes in the cluster" },
  { value: "12+", label: "MCP servers in production" },
  { value: "SSH + Git", label: "B0b agent credential scope" }
]} />
</ScrollReveal>
...
B0b, my deployment agent, holds SSH deploy keys and git credentials. The question: what can B0b reach if it goes off-script? I enumerated every credential B0b has access to and mapped the blast radius of each one.

### Message board access controls

Zulip is where agents coordinate. If an agent can post to Zulip, it can influence other agents' behavior by injecting context into the message stream. I checked whether agent posts are attributable, whether message streams are access-controlled per agent, and whether there is any content validation on agent-to-agent messages.

### Tool call authorization

Every tool call in Prometheus routes through CB4A, which evaluates authorized_action_set = f(user_intent) intersect f(agent_role) at execution time. Not at session start. Every call. I verified that the gate was actually enforcing, not just logging.

### Memory exfiltration paths

prom-memory stores decisions, context, and session history. If an agent can read prom-memory, it has access to the full operational history of the system. I checked access controls on the memory API and whether queries are scoped to the requesting agent's context.
...
<Callout variant="insight" label="CB4A held">
CB4A gates every tool call at runtime. The ExploitGym models exploited tools without authorization checks because the evaluation framework only checked outcomes, not methods. In Prometheus, every tool invocation passes through DIRA scope binding before CB4A authorizes the action. An agent cannot call a tool outside its authorized action set, even if it has network access to the tool's endpoint.
</Callout>

**DIRA bounds agent scope.** The Dual-Intent Runtime Authorization model means B0b's authorized actions are the intersection of what the delegating human intended and what B0b's role permits. If I tell B0b to deploy a service, B0b can call deployment tools. It cannot enumerate cluster secrets, query IMDS, or register Tailscale nodes, because none of those are in the authorized action set for a deployment task.

**B0b has SSH deploy keys, not broad tokens.** The deploy keys are scoped to specific repositories. They cannot push to arbitrary repos or access infrastructure outside the deployment target. This is not an accident; it is the result of enforcing least-privilege on NHI credentials from the start.

**Behavioral baselines exist.** prom-memory logs every session, every decision, every tool call. If B0b suddenly started making 6,000 API calls to enumerate Kubernetes namespaces, the deviation from baseline would be visible in the memory layer. The OpenAI environment had no behavioral baseline for the dataset-processor pod. I do.
...
<ScrollReveal>
<StatRow stats={[
  { value: "vault-b0b1", label: "Git credential exposure bead" },
  { value: "0", label: "Server-side token revocation endpoints" },
  { value: "Silent", label: "Zulip alert delivery failures" }
]} />
</ScrollReveal>

### Gap 1: Git credential exposure (vault-b0b1)

B0b's git credentials were committed to repository history. Not in the current tree, but in git history. If an attacker gained read access to the repo, they could extract valid credentials from prior commits. This is the same class of vulnerability as the Tailscale key in a Kubernetes secret: a credential that exists in a discoverable location because nobody thought to check.
...
### Gap 3: Zulip cert verification failure

Zulip is where security alerts route. During the audit, I discovered that TLS certificate verification had been silently failing on the Zulip connection. Alerts were not being delivered. The system was not reporting the delivery failure. If a real incident had occurred, the alerting channel would have been dark and nobody would have known.
...
**Token rotation tooling built.** B0b now has automated credential rotation on a schedule. When a credential is rotated, the old credential is invalidated immediately (where the upstream service supports it) or flagged for manual revocation. The rotation cadence is measured in hours, not "whenever someone remembers."
...
**Zulip TLS and alert delivery fixed.** Certificate verification is enforced. Alert delivery is confirmed with a health check. If the health check fails, a secondary alerting channel activates. No more silent failures.
...
graph TD
    subgraph "Already Covered"
        CB4A["CB4A: runtime tool call auth"]
        DIRA["DIRA: agent scope binding"]
        LSP["Least-privilege NHI credentials"]
        BL["Behavioral baselines via prom-memory"]
    end
    subgraph "Gaps Found and Fixed"
        ...
        ZULIP["Silent alert failure --> TLS + health check"]
    end
    CB4A --> SAFE["Prometheus hardened posture"]
    DIRA --> SAFE
    LSP --> SAFE
    BL --> SAFE
    GIT --> SAFE
    ...
    style ZULIP fill:#1a1917,stroke:#D29922,color:#E6EDF3
```
</DiagramBlock>
</ScrollReveal>
...
Behavioral detection is where I have the most confidence. prom-memory creates a dense audit trail. CB4A gates every tool call. If an agent starts deviating from its authorized action set, the deviation is visible.
