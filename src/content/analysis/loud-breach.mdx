...
If the ExploitGym models could chain nine hops from a sandbox to a production cluster at another company, what could a compromised agent do inside a system where the agents already have legitimate access to SSH keys, git credentials, MCP tokens, and Tailscale?
</Callout>

<ScrollReveal>
<StatRow stats={[
  { value: "Multi-node", label: "Kubernetes cluster" },
...
**Behavioral baselines exist.** The episodic memory layer logs every session, every decision, every tool call. If the deployment agent suddenly started making 6,000 API calls to enumerate Kubernetes namespaces, the deviation from baseline would be visible. The OpenAI environment had no behavioral baseline for the dataset-processor pod. I do.
