...
date: 2026-08-30
tags: ["mcp", "security", "agentic-ai", "model-context-protocol", "prometheus"]
featured: false
---
...
I run 12+ MCP servers in production on a K3s cluster. They expose tools for episodic memory, bead-based project tracking, vector search, Tailscale fleet management, Excalidraw diagramming, and more. My agents use these tools daily. This is not a spec critique from the sidelines. This is an operator report.

<ScrollReveal>
<StatRow stats={[
  { value: "12+", label: "MCP servers in production" },
  { value: "Daily", label: "Agent tool call frequency" },
  { value: "1", label: "Token already leaked (vault-b0b1)" }
]} />
</ScrollReveal>
...
I built audit logging into prom-memory's access layer because I think about this for a living. Most MCP server implementations do not log at this level. If you are running an MCP server in production without per-request audit logging, you have no forensic capability at the tool call layer.
...
```mermaid
graph TD
    TOKEN["Bearer Token"] --> T1["prom_memory_search"]
    TOKEN --> T2["prom_memory_add"]
    TOKEN --> T3["rag_search"]
    TOKEN --> T4["beads_create"]
    TOKEN --> T5["beads_close"]
    TOKEN --> T6["ts_exec"]
    TOKEN --> T7["ts_transfer"]
    TOKEN --> T8["vault_search"]
...
The spec will not save you. The spec defines a protocol. Security at the deployment layer is your problem. Here is what I have built or am building for Prometheus, and what you should build for yours.
...
5 to 15 minutes for high-privilege tokens. No longer. If your token can call `ts_exec` or any tool that touches infrastructure, 15 minutes is the ceiling. If you are running 24-hour or 7-day token TTLs on MCP servers that expose infrastructure tools, stop reading and go fix that.
</Callout>
...
Every tool call. Timestamp, client identity, token fingerprint, tool name, parameters, response status. This is your forensic capability. Without it, a compromised MCP server is a black box during incident response. I log every tool call through prom-memory. You need equivalent coverage.
