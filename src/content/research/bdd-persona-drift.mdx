...
excerpt: "At low temperature and short output lengths, the model is nearly deterministic at the token level but highly variable at the bigram-set level. The measurement unit doesn't fit the signal structure."
date: 2026-09-01
...
<PullQuote>At low temperature and short output lengths, the model is nearly deterministic at the token level but highly variable at the bigram-set level. The measurement unit doesn't fit the signal structure.</PullQuote>
...
```mermaid
flowchart LR
    A([Every 6h]) --> B[5 Canonical Prompts]
    B --> C[Inference Model]
    C --> D[Response Corpus]
...
<Callout variant="note">D53 enforcement is active. Five-run minimum window is met. Probe has run clean across all valid intervals since deployment. A model update triggers the gate; it does not bypass it.</Callout>
...
- [In Plain Language](/research/cognitive-security-part7) — the behavioral constraint library that BDD verification runs against. The Elliot Probe is the detection layer; the constraint library defines what it's checking for.
- [The Checklist](/research/cognitive-security-part6) — the operational posture that structural detection supports. BDD is how the checklist becomes executable.
- [Whose Memory Is It](/research/cognitive-security-part5) — multi-agent contamination as a detection target. The Elliot Probe's quarantine architecture addresses the high-volume-writer problem described here.
- [the memory layer: building episodic memory for an AI system that actually remembers](/build-logs/prom-memory) — the memory system the Elliot Probe monitors. The structural baseline is built from its fact type taxonomy.
- [Hardening an Autonomous Agent Fleet](/build-logs/agent-fleet-security-hardening) — security hardening at the infrastructure layer. Behavioral detection and network controls are complementary layers.
