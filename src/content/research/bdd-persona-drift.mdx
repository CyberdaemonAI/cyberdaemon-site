...
The replacement measures output structure instead of content.

At a given temperature and output length, a model's structural characteristics — how it organizes responses — are more stable across runs than its specific bigram vocabulary. A model that has drifted behaviorally will show changes in structural properties before it shows detectable change in bigram patterns. The imitation breaks at the biology level before the surface behavior shifts.
...
Domain-agnostic. Orthogonal properties. Three features, not more — nine runs and three features gives 27 data points at baseline, which is enough to set two-sigma bands without the numbers being dominated by outliers. I wanted a signal, not a statistics requirement.
...
Detection runs per-feature. For each feature, compute the rolling mean and standard deviation over the history window. Any single feature outside its two-sigma band triggers an alert, with which feature drifted, current value versus rolling mean, direction, and recommended action. Operators get diagnostic information. Not just a binary alarm.
