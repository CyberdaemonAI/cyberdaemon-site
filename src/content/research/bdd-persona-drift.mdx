...
sequenceDiagram
    ...
    participant B as Rolling Baseline (last 20 runs)
    participant G as Authorization Gate

    S->>M: 5 canonical probes
    M-->>F: Response corpus (~640 tokens)
    F->>F: avg_len · lex_div · sentences_per_resp
    F->>B: Current feature values
    B->>B: Compare to 2σ bands per feature
    alt All features within 2σ
        B-->>S: Pass — no drift detected
    else Any feature outside 2σ
        B->>G: Drift flag (feature + direction)
        G-->>G: CONTEXT_DRIFT gate fires
    end
```
</DiagramBlock>
