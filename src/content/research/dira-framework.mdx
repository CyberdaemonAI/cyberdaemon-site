...
personal time and equipment. The reference implementation runs in the Prometheus system, a
production AI agent stack on a homelab K3s cluster. This is not a whitepaper. It is a
framework derived from what actually happened when agentic systems ran unsupervised for
months, and what the controls caught.
</Callout>
...
```mermaid
graph TB
    subgraph "Layer 1: Behavioral Trust"
        ...
        F --> G[Gate D53: CONTEXT_DRIFT]
    end

    subgraph "Layer 2: JWT Scope Enforcement"
        H[JWT Validation] --> I[RS256 Signature Check]
        I --> J[Scope-per-Endpoint]
        J --> K[Route Handler Constraints]
    end

    subgraph "Layer 3: NHI Identity"
        L[Keycloak Client Credentials] --> M[Scoped JWT Issuance]
        M --> N[Secret Rotation via ESO]
    end
...
consistently with its declared purpose?* It runs in daemon-guard, a Kubernetes deployment
in the Prometheus namespace. Two detection scopes operate here:
...
The scope check asks: does the token include `watchdog:issues:create`? The route handler
asks: given this `POST /issues` request, is the `created_by` field being written
server-side from the JWT subject? Is the label being forced to match the JWT subject
identity? These resource-level constraints cannot be encoded in a scope string. They live
in the route handler. Both checks must pass.

**Layer 3: NHI identity management.** This layer answers: *who is this actor, and how was
its identity provisioned?* Each agent is a Keycloak client using client credentials flow
(RFC 6749 Section 4.4). No human login path exists. Scopes are provisioned
administratively. The secret rotation chain runs through HashiCorp Vault, External Secrets
Operator, Kubernetes Secret, environment variable. The agent never handles its own secret
provisioning.
...
## The Watchdog: DIRA in production

Theory is cheap. Here is what it looks like running.

The Watchdog is a Kubernetes CronJob that runs every 30 minutes. It authenticates to
Keycloak via client credentials, obtains a JWT with three scopes
(`watchdog:issues:create`, `watchdog:issues:close`, `prometheus:read`), runs cluster
health checks, and creates or closes issues in the Prometheus API based on the results.
...
Three things the Watchdog cannot do, enforced by the system:

**It cannot create an issue with any label other than `watchdog`.** The route handler for
`POST /issues` writes the label from the JWT subject. The request body's label field is
ignored. Invariant II: the Watchdog cannot produce resources that carry a different
...
gate.** If BDD detects that the Watchdog's output patterns have shifted significantly, Gate
D53 (CONTEXT_DRIFT) fires. Invariant I: declared scope and runtime scope are evaluated
independently.

<ScrollReveal>
<DiagramBlock caption="Watchdog authorization flow: three invariants enforced across three layers">
```mermaid
sequenceDiagram
    participant W as Watchdog CronJob
    participant KC as Keycloak
    participant API as Prometheus API
    participant DG as daemon-guard (BDD)
...
evaluation), detects the divergence independently of the scope check. Gate D53
(CONTEXT_DRIFT) fires on behavioral deviation.
...
```mermaid
graph TD
    H[Human Principal] -->|"Declares intent at instruction time"| I[Intent Declaration]
    I -->|"Scoped delegation"| A[Agent Principal]
    A -->|"Runtime action"| T[Tool Call]
    T --> V{DIRA Evaluation}
    V -->|"Check 1"| S[Token scope valid?]
    V -->|"Check 2"| R[Resource-level constraint met?]
    V -->|"Check 3"| B[Behavioral baseline consistent?]
    S --> D{All three pass?}
    R --> D
    B --> D
    D -->|"Yes"| P[Permit]
...
The reference implementation runs in the Prometheus system on a K3s cluster: two nodes
(NVIDIA Orin AGX, Beelink mini-PC), running Keycloak, daemon-guard, prometheus-api, and a
fleet of agent actors. Some implementation details that turned out to matter more than
expected:

**Separate the scope check from the route handler.** These are different jobs doing
different things. The scope check (`scope_check.py` middleware) validates action-type
authorization. The route handler validates resource-level constraints. If you merge them,
you lose the ability to reason about each independently. When an authorization failure
occurs, you need to know whether the agent lacked the scope (Layer 2 middleware) or had the
scope but violated a resource constraint (Layer 2 route handler) or had both but was
behaviorally drifting (Layer 1). Separation of concerns is not a style preference here. It
...
**The LLM is not a hard control.** The three-pass content evaluation pipeline in
daemon-guard uses hard regex rules as the floor, LLM classification for the gray zone, and
an adversarial re-pass on what the LLM cleared. The LLM never overrides a hard rule block.
This ordering is non-negotiable. LLMs produce inconsistent verdicts across prompt
framings. Using an LLM as the sole authorization gate is using a probabilistic system for a
deterministic job.

**Gate D53 starts in detect mode.** The behavioral drift gate (CONTEXT_DRIFT) currently
fires alerts and writes verdicts to memory but does not block active sessions. Promotion to
hard block requires 30 days of false-positive rate data. Shipping a blocking behavioral
gate without calibration data is shipping a denial-of-service vulnerability against your
own agents.
...
DIRA, the Prometheus system, and all original research described here were developed on
personal time and equipment. Views expressed are his own.*
