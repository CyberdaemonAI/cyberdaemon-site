...
excerpt: "Both approaches assume memory value decays monotonically with age. That assumption is wrong."
date: 2026-08-29
tags: ["ai-systems"]
...
flowchart LR
    ...
    subgraph bands["Band Classification"]
        C([Foundation Band])
        D([Middle Band])
        E([Active Band])
    end
...
The reference implementation runs against an episodic memory layer that stores structured
memories with embeddings generated on write. The decay tagging system maps directly to
Middle-Out bands:
...
The three-stage algorithm runs as a scheduled job. After each compression run, the
retrieval miss rate (how often a relevant memory was compressed away) feeds back into
the cluster granularity parameter. The system gets better at compression over time by
measuring what it lost.
...
- [the memory layer: building episodic memory for an AI system that actually remembers](/build-logs/prom-memory) — the production memory system that Middle-Out compression runs inside. Architecture, fact type taxonomy, and the automated compression pipeline.
- [The Notebook That Edits Itself](/research/cognitive-security-part1) — cognitive security implications of memory systems that compress and curate. What gets lost in compression shapes what gets surfaced at session start.
- [PADCN: a 5-dimensional affective state model for AI personas](/research/padcn-architecture) — the persona layer that sits above memory. Compression preserves behavioral context that PADCN reads at session initialization.
- [Writing to the Next Session](/build-logs/writing-to-the-next-session) — session handoff architecture. Middle-Out determines which prior session context survives into the next recovery path.
