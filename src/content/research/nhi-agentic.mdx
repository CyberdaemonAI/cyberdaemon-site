...
```mermaid
graph TD
    A[Non-Human Identities] --> B[Service Accounts]
    A --> C[API Keys]
    A --> D[OAuth Client Credentials]
    A --> E[Workload Certificates]
    A --> F[CI/CD Tokens]
    A --> G[Bot Accounts]
    A --> H[AI Agent Identities]
...
```mermaid
graph TD
    A[Zero Trust Policy Engine] --> B{Identity Verified?}
    B -->|Human| C[MFA + SSO + UEBA]
    B -->|Agent NHI| D[??? ]
    A --> E{Device Verified?}
    E -->|Human| F[EDR + Posture Check]
    E -->|Agent NHI| G[No device to check]
    A --> H{Context Verified?}
    H -->|Human| I[Location + Time + Risk Score]
    H -->|Agent NHI| J[No location. Ephemeral session.]
    A --> K{Continuous Verification?}
    K -->|Human| L[Session re-evaluation]
    K -->|Agent NHI| M[Token expires or it doesn't]
```
</DiagramBlock>
</ScrollReveal>
