...
<DiagramBlock caption="NHI governance overview: four-phase lifecycle that most organizations skip entirely">
```mermaid
flowchart LR
    A([AI Agent]) --> B[Inventory]
    B --> C[Provisioning]
    C --> D[Runtime]
    D --> E[Deprovisioning]
    E -->|Next agent| B
    D -->|Anomaly| F([Escalate to human])
```
</DiagramBlock>
...
**Security architecture chain** — NHI is the identity layer that makes authorization frameworks possible:

- [Zero Trust Applied to AI Systems](/research/zt-ai-systems) — NIST SP 800-207 applied to agents. ZTA defines the trust model; NHI governance is the prerequisite.
- [Dual-Intent Runtime Authorization (DIRA)](/research/dira-framework) — the authorization framework built on top of NHI. You cannot verify intent without a stable identity to attach it to.
- [Human Above the Loop](/analysis/human-above-the-loop) — NHI governance shifts human responsibility from approval to policy authoring. This article works through what that means in practice.
- [The tool permissions your agent doesn't need](/build-logs/agent-tool-permissions-attack-surface) — least privilege is the operational expression of NHI governance. An audit of what happens when it lapses.
- [Hardening an Autonomous Agent Fleet](/build-logs/agent-fleet-security-hardening) — egress controls, credential monitoring, and what NHI lifecycle management looks like in a running Kubernetes cluster.
