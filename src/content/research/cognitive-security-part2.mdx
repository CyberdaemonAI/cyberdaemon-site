...
excerpt: "The agent wasn't trying to plant anything. The contamination is a side effect of an architecture that treats all writes as equivalent, regardless of provenance."
date: 2026-09-01
...
flowchart TD
    ...
    TD --> P
    ND --> P
    P --> R[Retrieval without source attribution]
```
</DiagramBlock>
...
The lineage and attribution tooling described here is built on [the memory layer](/build-logs/prom-memory). The structural detection layer is described in [The Elliot Probe](/research/bdd-persona-drift).
