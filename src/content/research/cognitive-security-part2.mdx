...
date: 2026-09-01
tags: ["cognitive-security", "prom-memory", "ai-security", "research", "memory", "methodology"]
...
Each individual memory write was a legitimate operation. The source agent was doing its job. The content was accurate at the time of writing. The invocation of `prom_memory_add` looked exactly like every other invocation of `prom_memory_add`. Nothing triggered an alarm, because nothing was alarming in isolation.
...
<PullQuote text="The agent wasn't trying to plant anything. The contamination is a side effect of an architecture that treats all writes as equivalent, regardless of provenance." />
...
<Callout type="note">The architecture still produces contamination. These tools make it auditable. This is sanitation infrastructure, not a cure. The contamination rate is now a metric. Watch it over time.</Callout>
...
<Callout type="note">Part 3 covers the recursion problem — I used the AI to audit the AI — what that means, and what the maintenance schedule looks like from here.</Callout>
