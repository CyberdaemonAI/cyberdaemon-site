...
date: 2026-09-01
tags: ["cognitive-security", "ai-security", "research", "bdd", "constraints"]
...
The version he can't game: "source_agent must match one of the following literals — casey, claude-collaborative, claude-unilateral — or one of the following prefixes: harvest-auto, agent-*, system-*, persona-*. Any other value is a validation error."
...
```mermaid
flowchart LR
  O[Observable] --> V[Verifiable constraint]
  H[Human-readable] --> V
  I[Independently verifiable] --> V
  A[Adversary-resistant] --> V
...
This came from The automated agent flooding the layer with repetitive milestone writes from the same event. The cardinality cap I shipped — three milestone writes per session — was the first mitigation. The similarity threshold is the broader version. The 0.85 number is where I stopped getting false positives on my system. It's a parameter. Argue with it.
...
```mermaid
flowchart TD
  AB[Agent Bleed] -->|constraint| C1[source_agent taxonomy check]
  AL[Authority Laundering] -->|constraint| C2[decision attribution check]
  TD[Telemetry Drift] -->|constraint| C3[observation not preference]
  ND[Near-Duplicate Noise] -->|constraint| C4[pairwise similarity under 0.85]
  AS[Approval Bias] -->|constraint| C5[primer source diversity]
...
> **Given** a memory write with source_agent "agent-dispatcher"
...
```mermaid
flowchart LR
  V1[Version 1.0] -->|run against systems| R[Results]
...
The BDD verification framework that makes these constraints executable is described in [The Elliot Probe](/research/bdd-persona-drift). The memory architecture the constraints operate against is described in [the memory layer](/build-logs/prom-memory).
