---
title: "B0b: I Built an Autonomous AI Agent Fleet and 75% of It Broke"
...
B0b is not a theoretical architecture. It's four containerized AI agent instances
...
B0b solves this by treating each agent session as a fully isolated container. Own
...
B0b runs on a structured work item system. Each work item has an ID, a status, a
priority, and a description. The dispatch model is simple: work items go in, PRs come out.

<ScrollReveal>
<DiagramBlock caption="B0b fleet architecture: task dispatch to isolated agent instances">
```mermaid
graph TD
...
The original B0b design ran a persistent agent session in each instance, waiting for
...
**Spec enforcement.** The implementation spec document is what tells B0b what to build
for a given work item. Currently, spec existence is not enforced before dispatch. Adding
a gate (no spec, no dispatch) prevents B0b from starting work on an underspecified task
