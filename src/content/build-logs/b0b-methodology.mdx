...
excerpt: "An autonomous system that requires human intervention to authenticate is a contractor who can't unlock the job site."
date: 2026-08-30
...
flowchart TD
    ...
    C & D & E & F --> G([PR to GitHub])
    C & D & E & F --> H[(Memory Write)]
    C & D & E & F --> I([Ops Notification])
```
</DiagramBlock>
</ScrollReveal>
...
- [First fleet dispatch: a 25% success rate](/build-logs/b0b-first-fleet) — the failure report from the first run. The methodology article covers design; this covers what happened when it ran.
- [ISA-driven development: structured contracts for autonomous AI agents](/build-logs/isa-patterns) — the spec format that improved the fleet's success rate. Work contracts, not prompts.
- [My AI system builds itself, that's the point](/build-logs/ai-system-builds-itself) — the self-improving loop that the fleet architecture was built to enable. The methodology is the platform; this is what it shipped.
- [Hardening an Autonomous Agent Fleet](/build-logs/agent-fleet-security-hardening) — security hardening applied to the same fleet. The methodology article covers what the fleet is; the hardening article covers how to run it safely.
- [Agent-written CI: 18 of 19 tests](/build-logs/ci-gate-agent-written) — a successful fleet dispatch. One of the agents that completed its task autonomously, used the methodology described here.
