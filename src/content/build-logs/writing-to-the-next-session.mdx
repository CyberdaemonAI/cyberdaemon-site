...
{`graph LR
    A[Agent executes step] --> B[Updates task file]
    B --> C[Step complete]
    C --> D[Stop hook fires]
    D --> E[Reads task file]
    E --> F[Writes snapshot JSON]
    F --> G[Next session loads snapshot]
    G --> H[Reads next_action]
    H --> I[Executes without reconstruction]
