...
excerpt: "The agent treated a semantic mismatch as an arithmetic problem. The human recognized it as a grammar problem."
date: 2026-08-30
...
- [CB4A: content-based 4-tier authorization for AI tool calls](/build-logs/cb4a-implementation) — the authorization service whose tests the agent wired. The CI gate validates the enforcement logic.
- [ISA-driven development: structured contracts for autonomous AI agents](/build-logs/isa-patterns) — the work contract that specified this task. The agent's 18/19 success rate is a specification quality outcome.
- [When execution is cheap specification is the new skill](/analysis/execution-cheap-specification-skill) — the theoretical frame. The one test that failed required human understanding of a regex; the 18 that passed required precise specification.
- [I Built an Autonomous AI Agent Fleet and 75% of It Broke](/build-logs/b0b-methodology) — the fleet this agent ran in. The CI dispatch was one of the first successful runs after the initial 25% success rate.
