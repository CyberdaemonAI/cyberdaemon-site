...
flowchart TD
    ...
    subgraph policies["NetworkPolicy Stack - applied in order"]
        ...
        I["allow-internal: DNS, Pod CIDR, Service CIDR, LB VIP :443"]
        E["allow-external: :443 only, exclude RFC 1918"]
    end
...
<DiagramBlock caption="Security hardening sequence: credential expiry detection and alert flow">
```mermaid
sequenceDiagram
    participant Pod as Agent Pod
    participant Watcher as Expiry Watcher
    participant Ops as Ops Channel
    participant Human

    Pod->>Watcher: Start background watcher
    loop Every 30 minutes
        Watcher->>Watcher: Check token expiry timestamp
        alt Token expires within 60 min
            Watcher->>Ops: Alert: credential refresh needed
            Ops->>Human: Notification
            Human->>Pod: Provide refreshed token
        else Token valid
...
- [Non-Human Identity for Agentic Systems](/research/nhi-agentic): the identity governance framework that network hardening sits inside. Egress controls enforce least privilege at the network layer; NHI governance enforces it at the credential layer.
- [MCP's Security Problem Nobody's Writing About](/analysis/mcp-stateless-security): the protocol gaps that network controls partially compensate for. Deny-all egress limits what a compromised token can reach; it doesn't fix the stateless audit trail.
- [The tool permissions your agent doesn't need](/build-logs/agent-tool-permissions-attack-surface): the complementary access audit. Network controls restrict reachability; permission audits restrict what reaches.
- [The Loud Breach](/analysis/loud-breach): the incident that motivated this hardening sprint. What the containment audit found.
- [Zero Trust Applied to AI Systems](/research/zt-ai-systems): the architectural framework. Deny-all egress is ZTA applied at the network layer. Never trust the pod's origin; verify every outbound connection.
