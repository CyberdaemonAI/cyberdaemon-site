...
B0b (my build agent) opens a pull request. The bot sends me a Telegram message. I reply `/approve`. The PR merges. That was the whole loop.
...
<ScrollReveal>
Buttons are confirmed working. The first properly gated merge went through the full path: B0b opened the PR, bot sent the notification with CI status, I read the context, I tapped approve, the PR merged.
